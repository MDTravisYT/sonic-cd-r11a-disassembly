...
@GameInit:
	...
	jsr	LoadSoundDriver				; (ADDED) Load sound driver
	bsr.w	InitJoypads				; Initialize joypads

	move.b	#1,(v_time_zone).l			; (ADDED) Set time zone to present
	move.b	#0,(v_game_mode).w			; Set game mode to "level"

	move.b	(v_game_mode).w,d0			; Go to the current game mode routine
	andi.w	#$1C,d0
	;jmp	GameModes(pc,d0.w)
	jsr	GameModes(pc,d0.w)			; (ADDED)

	clr.w	(v_zone).l				; (ADDED) Reset zone ID
...
Pal_S1Title:						; Sonic 1 title screen (leftover)
	incbin	"level/unused/palettes/s1title.bin"
	even
...
@Blank:
	add.l	#LevelChunks,d1				; (ADDED) Add base chunk data pointer
...
StartZ80:
	move.w	#0,(Z80_BUS).l				; Start the Z80
	move	(v_saved_sr).w,sr			; Restore SR
	rts
...
PlayFMSound:
	...
	move.b	d0,(v_snd_queue_1).w			; Set ID in queue 1
	rts
...
@CheckQueue2:
	...
	move.b	d0,(v_snd_queue_2).w			; Set ID in queue 2
	rts
...
@CheckQueue3:
	...
	bne.s	@End					; If so, branch
	move.b	d0,(v_snd_queue_3).w			; Set ID in queue 3
...
LevelStartLoc:
	incbin	"level/level/startpos.bin"
	even
...
GetBlockDataAbsXY:
	...
	add.w	d4,d3
	add.w	d5,d3
	add.l	#LevelChunks,d3				; (ADDED) Add base chunk data pointer
	movea.l	d3,a0
...
LevelObjectIndex:dc.w	Objects_PPZ1A-LevelObjectIndex
	...
	dc.w	$FFFF, 0, 0, 0
Objects_PPZ1A:
	incbin	"level/level/objects.bin"
	even
...
; End of function ObjPlatform_Subtype$
...
ArtNem_AgedTeleporter:
	...
	even
ColAngleMap:
	incbin	"level/level/colangles.bin"
	even
ColHeightMap:
	incbin	"level/level/colheights.bin"
	even
ColWidthMap:
	incbin	"level/level/colwidths.bin"
	even
LevelCollision:
	incbin	"level/level/colblocks.bin"
	even
LevelLayoutIndex:
	...
	dc.w LevelLayout_Null-LevelLayoutIndex
LevelLayout_FG:
	incbin	"level/level/levelfg.bin"
	even
LevelLayout_BG:
	incbin	"level/level/levelbg.bin"
	even
LevelLayout_Null:
...
ArtNem_LargeFan:
	...
	even
MapNem_LevelBlocks:
	incbin	"level/level/blocks.bin"
	even
ArtNem_LevelArt:
	incbin	"level/level/art.bin"
	even
Ani_Powerup:
